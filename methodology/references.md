# Methodological References — African Instability Index (AII)
...
This is the primary methodological authority for the AII pipeline. It is the gold standard
...
| IQR Winsorization to handle outliers (cap, do not delete) | §5.2 | `02_clean.py` — `IQR_MULTIPLIER = 1.5` (Tukey bounds) |
...
| Sensitivity analysis / robustness testing across methods | §8 | `05_robustness.py` (planned) |
...
- The Handbook treats the Benefit of Doubt (BoD) method as exploratory. We implement it as a
  first-class scoring method because the AII user (a researcher) understands its interpretation.
...
- Formulation: LP per country, maximise `sum(w_i * s_i)` subject to:
  - `sum(w_i) = 1`
  - `WEIGHT_MIN <= w_i <= WEIGHT_MAX` for all pillars (bounds: 0.05 to 0.25)
...
- First PC loadings taken as raw weights; sign flipped if majority are negative
- Loadings normalised to sum to 1 (used as convex combination weights)
...
Threshold: α ≥ 0.70 is the conventional minimum for acceptable internal consistency
...
- Values below 0.70 should trigger a review of which indicators belong in the pillar
...
with 10 indicators) will mechanically produce higher alpha than a small pillar (e.g. Pillar E
with 4 indicators), even if coherence is similar.
...
Tukey's fences cap extreme values to `[Q1 - k·IQR, Q3 + k·IQR]` where k = 1.5.
Winsorization retains the observation in the sample (unlike trimming) and prevents
outlier countries from compressing the entire normalization range.

**Implementation in `02_clean.py`:**
- Applied per indicator across all countries after aggregation
- `IQR_MULTIPLIER = 1.5` (Tukey's conventional threshold for "outside values")
- Cap, do not delete: countries at the fence receive the fence value
- Logged in `winsorisation` sheet with pre/post values
...
### Robustness Analysis (Stage 05 — not yet implemented)
...
Will be used to justify the MaxS SLSQP worst-case weight search and rank shift analysis.
...
logged here before the change is committed.*
